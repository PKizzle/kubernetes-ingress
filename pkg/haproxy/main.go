...
func New(osArgs utils.OSArgs, env env.Env, cfgFile []byte, p process.Process, client api.HAProxyClient, rules rules.Rules) (h HAProxy, err error) {
	...
	if client == nil {
		h.HAProxyClient, err = api.New(h.CfgDir, h.MainCFGFile, h.Binary, h.RuntimeSocket, api.WithDynamicBackendManagementDisabled(osArgs.DisableDynamicBackendManagement))
		if err != nil {
